...
export const DashboardShell: React.FC<DashboardShellProps> = ({ children }) => {
  ...
  return (
    <div
      className="min-h-screen flex bg-[var(--background)] text-[var(--foreground)] relative overflow-hidden"
      dir={direction}
    >
      ...
      <div className="fixed inset-0 -z-10 pointer-events-none bg-[var(--background)]">
        ...
        <div className="absolute top-[-10%] start-[-10%] w-[40%] h-[40%] rounded-full bg-[#1A365D] dark:bg-[#0F172A] mix-blend-multiply dark:mix-blend-screen filter blur-[100px] opacity-70 animate-blob" />
        <div className="absolute top-[20%] end-[-10%] w-[35%] h-[35%] rounded-full bg-[#008080] dark:bg-[#0D9488] mix-blend-multiply dark:mix-blend-screen filter blur-[120px] opacity-50 animate-blob animation-delay-2000" />
        <div className="absolute bottom-[-20%] start-[20%] w-[50%] h-[50%] rounded-full bg-[#B8860B] dark:bg-[#D97706] mix-blend-multiply dark:mix-blend-screen filter blur-[150px] opacity-40 animate-blob animation-delay-4000" />
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen relative overflow-hidden">
        ...
        <footer className="h-11 border-t border-[var(--border)] bg-[rgba(15,23,42,0.15)] backdrop-blur-md flex items-center justify-between px-6 text-[10px] text-[var(--text-muted)] font-semibold select-none shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span>
              {isRtl
                ? "کلیه سامانه‌ها عملیاتی هستند"
                : "All services operational"}
            </span>
          </div>
          <div className="hidden sm:flex items-center gap-5">
            <span>
              {isRtl ? "بروزرسانی شده: همین الان" : "Last synced: Just now"}
            </span>
            <span className="px-2 py-0.5 rounded bg-[var(--border)] text-[9px]">
              v3.0.0 (Shell)
            </span>
          </div>
        </footer>
        ...
        {helpOpen && (
          <div className="fixed inset-0 z-[120]">
            ...
            <motion.div
              ...
              className="absolute inset-y-0 end-0 w-96 max-w-[90vw] bg-[var(--card)] border-[var(--border)] border-s border-e shadow-2xl flex flex-col overflow-hidden text-start"
            >
              {/* Header */}
              <div className="px-6 py-4 border-b border-[var(--border)] flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                  <HelpCircle
                    className="text-[var(--sky-blue-500)]"
                    size={18}
                  />
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <span className="text-[9px] text-[var(--text-muted)]">
                          {isRtl
                            ? "اتصال از طریق پروتکل MCP"
                            : "Integrate using MCP server guides"}
                        </span>
                      </div>
                    </a>
              ...
              <div className="p-4 border-t border-[var(--border)] bg-[var(--background)]/20 text-center text-[10px] text-[var(--text-muted)] font-semibold shrink-0">
                <span>
                  {isRtl
                    ? "پشتیبانی ۲۴ ساعته سئورچبل"
                    : "Seorchable Support - 24/7 online"}
                </span>
              </div>
            </motion.div>
          </div>
